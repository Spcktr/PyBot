    ...
    async def pricecheck(self, ctx):
        ...
        for i in response["applist"]["apps"]:
            if (i['name'] == game):
                ...
                await session.close()
                price = (priced[f"{app}"]["data"]
                         ["price_overview"].get('final_formatted'))
                image = (priced[f"{app}"]["data"].get('header_image'))
                discount = (priced[f"{app}"]["data"]
                            ["price_overview"].get('discount_price'))

                # embed code start
                embed = discord.Embed(
                    description="Here is your result", color=3447003)
                embed.set_image(url=f"{image}")
                embed.add_field(
                    name=f'Current {game} price', value=f'{price}', inline=False)
                if discount is int('0'):
                    pass
                else:
                    embed.add_field(name="Current Discount",
                                    value=f'{discount}% Off')
                embed.set_footer(text="Price current as of {}".format(
